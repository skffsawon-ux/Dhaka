...
tmux send-keys -t mars:0.1 "ros2 launch maurice_sim_bringup sim_rosbridge.launch.py" C-m
echo "Started simulation..."
sleep 2  # Give rosbridge time to start up

# Create a new pane and run the nav system
tmux split-window -t mars:0.1 -h
tmux send-keys -t mars:0.2 "ros2 launch maurice_nav navigation_sim.launch.py" C-m
echo "Started navigation system..."
sleep 2  # Give nav system time to start up

# Create a new pane for the brain client
tmux split-window -t mars:0.0 -h
# Let's add a debug message to see which pane we're currently in
tmux send-keys -t mars:0.1 "ros2 launch brain_client brain_client.sim.launch.py" C-m
echo "Started brain client..."

# Select the first pane
tmux select-pane -t mars:0.1
