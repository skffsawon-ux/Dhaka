#!/usr/bin/env python3
"""
Maurice Robot System Detection Script
=====================================
...
def check_cameras():
    ...
    # Show other detected cameras for debugging
    other_cameras = [c for c in cameras if c != head_camera and c != arm_camera]
    if other_cameras:
        print(f"  {Colors.YELLOW}Other video devices:{Colors.END}")
        for cam in other_cameras:
            print(f"    - {cam['path']}: {cam['card']}")
...
def main():
    ...
    print(f"║          MAURICE ROBOT SYSTEM CHECK                      ║")
    print(f"╚══════════════════════════════════════════════════════════╝{Colors.END}")
