...
class PickMotor(Primitive):
    ...
    def execute(self):
        """
        Executes the pick_motor policy by calling the ExecuteBehavior action server.
        ...
        if not self.node:
            self.logger.error(
                "PickMotor primitive is not functional due to missing ROS node."
            )
        ...
        if not self._action_client:
            ...
            if not self._action_client:
                self.logger.error(
                    "PickMotor primitive could not create ExecuteBehavior action client."
                )
                return "Primitive could not create action client", PrimitiveResult.FAILURE

        self.logger.info(
            f" \033[96m[BrainClient] Calling ExecuteBehavior for picking up motor (blocking)\033[0m"
        )
        ...
        goal_msg.behavior_name = "pick_motor"
        ...
        if not result_future.done():
            ...
            if self._goal_handle:
                ...
                self._goal_handle.cancel_goal_async()
            return "Picking up motor action timed out", PrimitiveResult.FAILURE
        ...
        self._send_feedback("I should be looking at my wrist camera to check if I have picked up the motor successfully")

        if status == GoalStatus.STATUS_SUCCEEDED:
            final_result = result_response.result
            self.logger.info(f"Action succeeded! Result: {final_result.success}")
            if final_result.success:
                action_result_message = "Motor picked up successfully"
                primitive_status = PrimitiveResult.SUCCESS
            else:
                action_result_message = "Picking up motor action reported failure"
                primitive_status = PrimitiveResult.FAILURE
        elif status == GoalStatus.STATUS_ABORTED:
            self.logger.info("Goal aborted")
            primitive_status = PrimitiveResult.CANCELLED
            action_result_message = "Picking up motor aborted"
        elif status == GoalStatus.STATUS_CANCELED:
            self.logger.info("Goal canceled")
            action_result_message = "Picking up motor canceled"
            primitive_status = PrimitiveResult.CANCELLED
        else:
            self.logger.info(f"Goal failed with unknown status: {status}")
            action_result_message = (
                f"Picking up motor failed with unknown status: {status}"
            )
    ...
    def cancel(self):
        """
        Cancel the pick_motor operation.
        This is a best-effort cancellation.
        """
        if self._goal_handle:
            self.logger.info("\033[91m[BrainClient] Canceling pick_motor operation \033[0m")
            self._goal_handle.cancel_goal_async()
            return "\033[92m[BrainClient] Cancellation request sent for picking up motor. \033[0m"
        else:
            self.logger.info(
                "\033[91m[BrainClient] Pick up motor operation cannot be canceled as no goal is active. \033[0m"
            )
            return "\033[91m[BrainClient] No active pick_motor operation to cancel. \033[0m" 