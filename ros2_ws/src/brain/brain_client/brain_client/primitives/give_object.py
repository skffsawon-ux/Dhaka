...
class GiveObject(Primitive):
    ...
    def execute(self):
        """
        Executes the give_object policy by calling the ExecuteBehavior action server.
        ...
        if not self.node:
            self.logger.error(
                "GiveObject primitive is not functional due to missing ROS node."
            )
        ...
        if not self._action_client:
            ...
            if not self._action_client:
                self.logger.error(
                    "GiveObject primitive could not create ExecuteBehavior action client."
                )
                return "Primitive could not create action client", PrimitiveResult.FAILURE

        self.logger.info(
            f" \033[96m[BrainClient] Calling ExecuteBehavior for giving object (blocking)\033[0m"
        )
        ...
        goal_msg.behavior_name = "give_object"
        ...
        if not result_future.done():
            ...
            if self._goal_handle:
                ...
                self._goal_handle.cancel_goal_async()
            return "Giving object action timed out", PrimitiveResult.FAILURE
        ...
        self._send_feedback("I should be looking at my wrist camera to check if I have given the object to the user successfully")

        if status == GoalStatus.STATUS_SUCCEEDED:
            final_result = result_response.result
            self.logger.info(f"Action succeeded! Result: {final_result.success}")
            if final_result.success:
                action_result_message = "Object given to user successfully"
                primitive_status = PrimitiveResult.SUCCESS
            else:
                action_result_message = "Giving object action reported failure"
                primitive_status = PrimitiveResult.FAILURE
        elif status == GoalStatus.STATUS_ABORTED:
            self.logger.info("Goal aborted")
            primitive_status = PrimitiveResult.CANCELLED
            action_result_message = "Giving object aborted"
        elif status == GoalStatus.STATUS_CANCELED:
            self.logger.info("Goal canceled")
            action_result_message = "Giving object canceled"
            primitive_status = PrimitiveResult.CANCELLED
        else:
            self.logger.info(f"Goal failed with unknown status: {status}")
            action_result_message = (
                f"Giving object failed with unknown status: {status}"
            )
    ...
    def cancel(self):
        """
        Cancel the give_object operation.
        This is a best-effort cancellation.
        """
        if self._goal_handle:
            self.logger.info("\033[91m[BrainClient] Canceling give_object operation \033[0m")
            self._goal_handle.cancel_goal_async()
            return "\033[92m[BrainClient] Cancellation request sent for giving object. \033[0m"
        else:
            self.logger.info(
                "\033[91m[BrainClient] Give object operation cannot be canceled as no goal is active. \033[0m"
            )
            return "\033[91m[BrainClient] No active give_object operation to cancel. \033[0m" 